resample_function_weib =function(mean,sd,N=1000) {
set.seed(1000);
                   k <- (sd/mean)^(-1.086);
                   c <- mean/(gamma(1+1/k));
                   rweibull(N,shape=k, scale=c)
                   }


summary_large=function(x) {data.frame(c(pastecs::stat.desc(as.numeric(x)),
                                        data.frame(skew=as.numeric(psych::describe(x[[10]]))[11],
                                                   kurtosis=as.numeric(psych::describe(x))[12])))
}

descdist2=function (data, discrete = FALSE, boot = NULL, method = "unbiased", 
    graph = TRUE, obs.col = "darkblue", obs.pch = 16, boot.col = "orange", title="prova") 
{
